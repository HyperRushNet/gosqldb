...
async def create_room(room: Room):
    ...
    if exists: raise HTTPException(400,"Room ID already exists")
...
async def add_item(room_id: str, item: Item):
    ...
    exists = await database.fetch_one(items.select().where(items.c.id==item.id, items.c.room_id==room_id))
    if exists: raise HTTPException(400,"Item ID already exists in this room")
